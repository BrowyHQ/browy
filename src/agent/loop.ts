...
17. **SPA state-change loop.** Modern SPAs (LinkedIn, Outlook, Notion, Twitter, Discord, Slack, Figma) often respond to a click WITHOUT changing the URL — they mutate the DOM in place after a network round-trip. After any click/type that *should* have changed state, treat the next \`inspect_page\` as proof. Look for: a new heading/title, a previously-disabled button now enabled (or vice-versa), the clicked element's \`pressed\`/\`expanded\`/\`selected\` state flipping, a modal/drawer appearing in the snapshot's element list, or a row count growing. If none of these appear, the action did NOT take effect — call \`wait_for(selector, 2000)\` for a stable post-state element, or \`scroll\` if the new content might be below the fold, then re-inspect. After two failed retries on the same element, switch strategy (different element from snapshot, \`evaluate_js\` to read the framework's state directly, or \`await_user\` if it looks like a permission/auth gate).
